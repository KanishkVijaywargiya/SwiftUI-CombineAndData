...
struct PrimaryButton: View {
    ...
    var lightModeButton: some View {
        ...
        .frame(height: 50)
        .background(
            LinearGradient(gradient: Gradient(colors: [Color(#colorLiteral(red: 0.337387681, green: 0.2152375281, blue: 0.88736552, alpha: 1)), Color(#colorLiteral(red: 0.6350373626, green: 0.5573855639, blue: 0.9254464507, alpha: 1))]), startPoint: .top, endPoint: .bottom)
        )
        .cornerRadius(16)
...
struct PrimaryButton_Previews: PreviewProvider {
    static var previews: some View {
        Group {
            PrimaryButton()
            PrimaryButton().environment(\.colorScheme, .dark)
        }
    }
}
